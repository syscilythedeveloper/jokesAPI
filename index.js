...
app.get("/jokes/:im", (req, res) => {
  const id = parseInt(req.params.im);
  console.log(id);

  const theJoke = jokes.find((joke) => joke.id === id);
  res.json(theJoke);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    console.log(jokes[searchIndex - 1]);
    res.sendStatus(200);
  } else {
    res
      .status(404)
      .json({ error: `Joke with id ${id} not found. Nothing got deleted` });
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    ...
    res.status(404).json({ error: `Ya can't` });
  }
});
